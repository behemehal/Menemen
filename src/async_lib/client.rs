...
#[cfg(feature = "https")]
use tokio_native_tls::{native_tls::TlsConnector, TlsConnector as TokioTlsConnector};
...
/// Async HTTP client bound to a target URL.
#[derive(Debug)]
pub struct Client {
    url: Url,
}
...
impl Client {
    ...
    pub async fn connect(&self, timeout_ms: u64) -> Result<TcpStream, RequestError> {
        ...
        let stream = timeout(Duration::from_millis(timeout_ms), TcpStream::connect(address))
            .await
            .map_err(|_| RequestError::ConnectionTimeout)??;
        Ok(stream)
    }
    ...
    pub async fn send_request(
        ...
        loop {
            let stream = Client::new(current_url.clone())
            ...
            let mut stream = if tls {
                #[cfg(feature = "https")]
                {
                    ...
                    let stream = TokioTlsConnector::from(connector)
                        .connect(&current_url.host, stream)
                        .await?;
